...
def affine_backward(dout, cache):
  ...
  N = x.shape[0]
  x_row = x.reshape(N, -1)
  dx__per_row = dout.dot(w.T)
  dx = dx__per_row.reshape(x.shape)
  dw = x_row.T.dot(dout)
...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    xhat = (x-mini_mean) /np.sqrt(mini_var+eps)
    out = gamma*xhat +beta

    cache = (x,gamma,beta, xhat,mini_mean,mini_var,eps)
    ...
    # ================================================================ #
    test_time_normal_act = (x-running_mean)/np.sqrt(running_var+eps)
    out = gamma*test_time_normal_act + beta
...
def batchnorm_backward(dout, cache):
  ...
  # ================================================================ #
  x, gamma, _, x_hat, minibatch_mean, minibatch_var, eps = cache
  N = x.shape[0]
  dgamma = np.sum(dout*x_hat, axis=0)
  dbeta = np.sum(dout, axis=0)
  dx_hat = dout*gamma
  dx = dx_hat/np.sqrt(minibatch_var+eps)
  dvar = np.sum(dx_hat*(x-minibatch_mean)*(-0.5)*(minibatch_var+eps)**(-1.5), axis=0)
  dmean = np.sum(dx_hat*(-1)/np.sqrt(minibatch_var+eps), axis=0)
  dmean += dvar*np.sum(-2*(x-minibatch_mean), axis=0)/N
  dx += dvar*2*(x-minibatch_mean)/N + dmean/N
